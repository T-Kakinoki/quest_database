...
|5|program_genre||番組がどのジャンルに該当するかを保存|
...
|channel_name|VARCHER(100)|NO|UNIQUE||NO|チャンネル名|
-ユニークキー制約:channelカラムに対して設定


2:番組テーブル
|カラム名|データ型|NULL|キー|初期値|AUTO INCREMENT|説明|
|program_id|INT|NO|PK||YES|番組ID|
|program_name|VARCHER(100)|NO|UNIQUE||NO|番組名|
|program_detail|TEXT|NO|||NO|番組詳細|
|program_type|VARCHER(10)|NO|||NO|単発orシリーズ|
...
|genre_name|VARCHER(100)|NO|UNIQUE||NO|ジャンル名|
...
-ユニークキー制約:genre_id、program_idの組み合わせに対して設定 
...
|broadcast_end_time|DATETIME|NO||||放送終了時間|
|view_count|INT|NO||||視聴数|
-外部キー制約:channel_id に対して channel テーブルから設定
-外部キー制約:episode_id に対して episode テーブルから設定
ユニークキー制約:channel_id、episode_id、broadcast_timeの組み合わせに対して設定
##ER図
